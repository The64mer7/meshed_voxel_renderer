...
{
    return std::to_string(key.x) + sep + std::to_string(key.y) + sep + std::to_string(key.z) + sep +
           std::to_string(key.w);
}
...
constexpr size_t Bytes(size_t bytes) { return bytes; }

constexpr size_t KB(size_t kilobytes) { return kilobytes * 1024ull; }

constexpr size_t MB(size_t megabytes) { return megabytes * 1024ull * 1024ull; }

constexpr size_t GB(size_t gigabites) { return gigabites * 1024ull * 1024ull * 1024ull; }
...
public:
    ...
    void create(size_t size) { m_MaxSize = size; }
    ...
    void debug_print(uint64_t blockSize = KB(4))
    {
        ...
        while (it != m_OffsetToSize.end() && nextIt != m_OffsetToSize.end())
        {
            ...
            if (!(it->second + it->first == nextIt->first))
            {
                for (uint64_t i = 0; i < ((nextIt->first - (it->first + it->second)) / blockSize);
                     i++)
                {
                    std::cout << id << "id )" << freeSlot;
                    FnTryNewLine();
    ...
    void debug_log(std::string& out, uint64_t blockSize)
    {
        ...
        if (totalSize < KB(4))
            ...
            out += std::format("Memory usage: {} / {}MB", totalSize / 1024 / 1024,
                               m_MaxSize / 1024 / 1024);
    ...
    void for_each_memory_block(
        std::function<void(offset_t offset, size_t size, bool is_allocated)> for_each)
    {
        std::lock_guard lock(m_RecMtx);
        auto it = m_OffsetToSize.begin();
        if (it == m_OffsetToSize.end())
...
private:
    const std::map<offset_t, size_t>& get_offsets_to_size_map() { return m_OffsetToSize; }

    const std::map<size_t, Offsets>& get_size_to_offsets_map() { return m_SizeToOffsets; }
