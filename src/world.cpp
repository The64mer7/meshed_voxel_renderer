...
void World::create(const WorldData& data, const OctreeClipmapGenerateSettings& settings)
{
    ...
    m_world_buffer.AllocateStorage(data.world_vram, nullptr,
...
void World::update(const glm::vec3& player_position, float fov)
{
    ...
    {
        bool past_range = glm::distance(player_position, m_last_update_pos) >
                          m_data.update_distance;
        if (past_range)
        {
            m_last_update_pos = player_position;
            double t0 = glfwGetTime();
            m_clipmap.generate_by_perspective(m_settings.min_depth, m_settings.max_depth,
...
void World::destroy() { m_edits.cleanup(); }
...
{
    m_clipmap.for_each_chunk_in_area(bounds, [out_chunks](const ChunkKey& key)
                                     { out_chunks->push_back(key); });
}
