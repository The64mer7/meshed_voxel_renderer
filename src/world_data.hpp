...
struct ChunkKey
{
    ...
    ChunkKey() = default;
    ChunkKey(int x, int y, int z, int lod) : x(x), y(y), z(z), lod(lod) {}
    ChunkKey(glm::ivec4 v) : raw(v) {}
...
inline bool operator==(const ChunkKey& lhs, const ChunkKey& rhs) { return lhs.raw == rhs.raw; }
...
{
    int world_size_exp;
    float world_size() const { return 1 << world_size_exp; }

    float chunk_size(int32_t lod) const { return glm::ldexp(1.f, world_size_exp - lod); }

    float voxel_size(int32_t lod) const { return chunk_size(lod) / voxels_per_chunk_axis; }
