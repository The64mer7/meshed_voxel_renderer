...
public:
    ...
    void setShaderSource(const std::string& source) { m_source = source; }
    void cleanup()
    {
        if (m_handle != 0)
...
{
public:
    void bind() { glUseProgram(m_handle); }
    ...
    uint32_t getUniformLoc(const char* name) { return glGetUniformLocation(m_handle, name); }
    ...
    void uniformImg2D(const char* name, unsigned int texture_id, int tex_slot, unsigned int access,
                      unsigned int format)
    {
    ...
    void uniformImg3D(const char* name, unsigned int texture_id, int tex_slot, unsigned int access,
                      unsigned int format)
    {
        glBindImageTexture(tex_slot, texture_id, 0, GL_FALSE, 0, access, format);
        glUniform1i(getUniformLoc(name), tex_slot);
...
inline void dispatch_compute(ShaderProgramCompute& compute_program, glm::ivec3 groups,
                             GLbitfield barriers)
{
...
inline void dispatch_compute(ShaderProgramCompute& compute_program, uint32_t x, uint32_t y,
                             uint32_t z, GLbitfield barriers)
{
    compute_program.bind();
...
inline void dispatch_compute_indirect(ShaderProgramCompute& compute_program,
                                      GLintptr indirect_byte_offset, GLbitfield barriers)
{
    compute_program.bind();
    glDispatchComputeIndirect(indirect_byte_offset);
    if (barriers != 0)
    {
        glMemoryBarrier(barriers);
    }
