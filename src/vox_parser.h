...
struct ChunkSizeData
{
    ...
    int32_t y;
    int32_t z;
    int32_t volume() { return x * y * z; }
    int32_t index(int32_t cx, int32_t cy, int32_t cz) { return cx + cy * x + cz * x * y; }
    void print() { printf("size: %ux%ux%u\n", x, y, z); }
};
struct Voxel
{
    uint8_t x, y, z, i;
    void print() { printf("voxel: %u %u %u %u\n", x, y, z, i); }
};
...
static LoadResult load_vox(const std::string& filepath, std::vector<Voxel>* out_voxels,
                           std::vector<uint32_t>* out_voxel_colors, ChunkSizeData* out_data)
{
