...
struct UpdateGreedyMeshTask
{
    ...
    void operator()(ArenaAllocator* allocator)
    {
        ...
        for (size_t i = begin; i < end; i++)
        {
            ...
            if (!voxel_data->compute_terrain(key, *world_data, edits, structures, max_structures))
                continue;
            ...
            double elapsed_mesh_ms = std::chrono::duration<double, std::milli>(tmesh1 - tmesh0)
                                         .count();
            ...
            if (result.face_count > 0)
            {
                ...
                {
                    if (!(size_bytes % sizeof(GreedyFace) == 0))
                        LOG("MESHING ERROR");
                    if (!(offset_bytes % sizeof(GreedyFace) == 0))
                        LOG("MESHING ERROR");
...
struct BatchedTasks
{
    ...
    bool all_tasks_completed(std::atomic_uint32_t* counter) { return counter->load() == 0; }
};
...
private:
    ...
    void submit_tasks(OctreeClipmap::LeavesVector* chunks, bool remesh,
