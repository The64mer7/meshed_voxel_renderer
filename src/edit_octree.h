...
public:
    ...
    bool load(const std::string& filepath, const glm::vec3& position)
    {
        ...
        for (uint32_t i = 0; i < scene->num_instances; i++)
        {
            ...
            glm::mat4 transform_matrix(
                instance.transform.m00, instance.transform.m01, instance.transform.m02,
                instance.transform.m03, instance.transform.m10, instance.transform.m11,
                instance.transform.m12, instance.transform.m13, instance.transform.m20,
                instance.transform.m21, instance.transform.m22, instance.transform.m23,
                instance.transform.m30, instance.transform.m31, instance.transform.m32,
                instance.transform.m33);
            transform_matrix = glm::rotate(transform_matrix, glm::radians(-90.f),
                                           glm::vec3(1, 0, 0));
            ...
            for (uint32_t z = 0; z < model->size_z; z++)
            {
                for (uint32_t y = 0; y < model->size_y; y++)
                {
                    ...
                    {
                        uint32_t voxel_index = x + (y * model->size_x) +
                        ...
                        glm::vec4 local_pos(static_cast<float>(x), static_cast<float>(y),
                        ...
                        has_voxels = true;
                        glm::ivec3 chunk_coord = glm::floor(glm::vec3(world_pos) /
                        ...
                        if (chunk)
                        {
                            chunk->data[local_voxel_pos.z][local_voxel_pos.y][local_voxel_pos.x] =
                                quantized_color;
                        }
                    }
                }
    ...
    uint16_t get_voxel(const glm::vec3& p, float size) override
    {
        ...
        if (local_pos.x < 0 || local_pos.x >= CHUNK_SIZE || local_pos.y < 0 ||
            local_pos.y >= CHUNK_SIZE || local_pos.z < 0 || local_pos.z >= CHUNK_SIZE)
        {
            return 0;
        }
    ...
    bool is_destructive() override { return false; }
};
...
public:
    ...
    bool is_destructive() override { return flags == 1; }
};
...
public:
    ...
    uint32_t find_instances_in_region(const aabb3d& aabb, WorldInstance* out_instances,
    ...
    void debug_print() { debug_log(m_root, 0); }
...
private:
    ...
    void add_node_instance(OctreeNode* node, const aabb3d& node_bounds,
                           const aabb3d& structure_bounds, instance_id instance)
    {
        struct StackItem
        {
            OctreeNode* node;
    ...
    void insert_instance_to_octree(instance_id instance, OctreeStructure* structure,
    ...
    template <int rank>
    OctreeNode* find_smallest_node_for_each(const aabb3d& aabb, aabb3d* out_node_bounds,
                                            glm::ivec3* out_morton,
                                            std::function<void(OctreeNode*)> fn_for_each_descendant)
    {
        OctreeNode* node = m_root;
        aabb3d node_bounds = m_root_bounds;
        uint64_t depth = 0;
